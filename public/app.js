...
document.querySelectorAll('[data-auth]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-auth]').forEach(x=>x.classList.toggle('active',x===b));$('#loginForm').hidden=b.dataset.auth!=='login';$('#registerForm').hidden=b.dataset.auth!=='register';msg('')});
$('#loginForm').onsubmit=async e=>{e.preventDefault();try{const j=await api('/api/auth/login',{method:'POST',body:JSON.stringify({email:$('#loginEmail').value,password:$('#loginPassword').value})});token=j.token;localStorage.setItem('relay_token',token);await boot()}catch(err){msg(err.message)}};
$('#registerForm').onsubmit=async e=>{e.preventDefault();try{const j=await api('/api/auth/register',{method:'POST',body:JSON.stringify({email:$('#regEmail').value,name:$('#regName').value,password:$('#regPassword').value,inviteCode:$('#regInvite').value})});token=j.token;localStorage.setItem('relay_token',token);await boot()}catch(err){msg(err.message)}};
...
async function boot(){try{data=await api('/api/dashboard');me=data.user;authView.hidden=true;dash.hidden=false;$('#sideName').textContent=me.name;$('#sideEmail').textContent=me.email;$('#sideAvatar').textContent=me.name[0].toUpperCase();$('#topAvatar').textContent=me.name[0].toUpperCase();render('overview')}catch{localStorage.removeItem('relay_token');token=null}}
