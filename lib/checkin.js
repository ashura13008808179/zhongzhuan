...
export function sampleCheckInReward(rng = defaultRng) {
  let u = Number(rng());
  if (!Number.isFinite(u)) u = defaultRng();
  u = Math.min(Math.max(u, 0), 1);
  if (u >= 1) return CHECKIN_AMOUNTS[CHECKIN_AMOUNTS.length - 1];
  const idx = WEIGHTS.cumulative.findIndex((c) => u < c);
...
/** Inclusive desk window: 08:30–23:30 Asia/Shanghai. */
export const RECHARGE_DESK_START_MIN = 8 * 60 + 30;
export const RECHARGE_DESK_END_MIN = 23 * 60 + 30;
export const RECHARGE_HOURS_CLOSED_MSG = '每天早上 8:30 到晚上 11:30（北京时间）才会处理卡密充值。当前不在服务时间，提交的付款将在下一服务时段内核对发卡。';
export const RECHARGE_HOURS_OPEN_MSG = '每天早上 8:30 到晚上 11:30（北京时间）处理卡密充值。';

export function shanghaiHourMinute(date = new Date()) {
...
export function rechargeDeskOpen(date = new Date()) {
  const { minutes } = shanghaiHourMinute(date);
  return minutes >= RECHARGE_DESK_START_MIN && minutes <= RECHARGE_DESK_END_MIN;
}

export function publicRechargeHours(date = new Date()) {
  const open = rechargeDeskOpen(date);
  return {
    timezone: CHECKIN_TIMEZONE,
    start: '08:30',
    end: '23:30',
    open,
    message: open ? RECHARGE_HOURS_OPEN_MSG : RECHARGE_HOURS_CLOSED_MSG,
    closedMessage: RECHARGE_HOURS_CLOSED_MSG
  };
}
