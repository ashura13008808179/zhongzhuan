...
/** Beibeihai Claude-Kiro / welfare completion path often hangs; fail clients fast. */
export const KIRO_FAILFAST_TIMEOUT_MS = 30000;
export const KIRO_FAILFAST_TIMEOUT_MIN_MS = 20000;
export const KIRO_FAILFAST_TIMEOUT_MAX_MS = 35000;
export const CLAUDE_KIRO_PROVIDER_IDS = ['grp_claude_kiro', 'grp_claude_kiro_welfare'];
...
/** Beibeihai Claude-Kiro + welfare — prefer fail-fast over multi-minute anthropic+chat stacks. */
export function isClaudeKiroProvider(provider) {
  const id = String(provider?.id || '');
  return CLAUDE_KIRO_PROVIDER_IDS.includes(id);
}

export function providerPrefersAnthropicMessages(provider, model = '') {
  const id = String(provider?.id || '');
  // Kiro/welfare: skip anthropic-first (upstream /messages hangs the same as /chat).
  if (isClaudeKiroProvider(provider)) return false;
  if (id === 'grp_cc_max') return true;
...
export function providerFetchTimeoutMs(provider, { stream = false } = {}) {
  ...
  // Kiro fail-fast: honor configured hop timeout within a tight band (do not floor to 120s).
  if (isClaudeKiroProvider(provider)) {
    const base = configured > 0 ? configured : KIRO_FAILFAST_TIMEOUT_MS;
    const hop = Math.min(Math.max(base, KIRO_FAILFAST_TIMEOUT_MIN_MS), KIRO_FAILFAST_TIMEOUT_MAX_MS);
    if (stream) return Math.min(Math.max(hop, 60000), 120000);
    return hop;
  }
...
export function ensureProviderTimeouts(providers) {
  ...
  for (const provider of list) {
    ...
    if (isClaudeKiroProvider(provider)) {
      // Single hop only — stacking retries recreates multi-minute hangs.
      if (Number(provider.maxRetries) !== 0) {
...
/** Health-probe key name on vip1129 / beibeihai. Must stay stable. */
export function upstreamProbeKeyName(providerId) {
  return `relay-probe-${String(providerId || '')}`.slice(0, 60);
}

function listedRowId(row) {
  const id = row?.id ?? row?.key_id ?? row?.keyId ?? null;
  return id == null || id === '' ? null : String(id);
}

function listedRowName(row) {
  return String(row?.name || '');
}

function listedRowGroupId(row) {
  return row?.group_id ?? row?.groupId ?? row?.group?.id;
}

function listedRowSecret(row) {
  const key = row?.key || row?.secret || row?.api_key || row?.apiKey || null;
  if (!key) return null;
  return { id: listedRowId(row), key: String(key) };
}

function listedRowInGroup(row, groupId) {
  if (groupId == null || groupId === '') return true;
  const gid = Number(groupId);
  if (!Number.isFinite(gid)) return true;
  return Number(listedRowGroupId(row)) === gid;
}

function isRelayProbeName(name) {
  return String(name || '').includes('relay-probe');
}

/**
 * Pick at most one probe key to keep and list extras to delete.
 * Only exact-name `relay-probe-*` rows in the same group are deleted.
 * Business keys (names that do not include `relay-probe`) are never in deleteIds.
 */
export function planUpstreamProbeKeys(payload, { probeName, groupId, forceNew = false } = {}) {
  const rows = flattenListedKeys(payload);
  const wantName = String(probeName || '');
  const exact = rows.filter((row) => {
    if (!wantName) return false;
    if (!listedRowInGroup(row, groupId)) return false;
    const name = listedRowName(row);
    return name === wantName && isRelayProbeName(name);
  });

  const deleteIds = [];
  const seen = new Set();
  const pushDelete = (row) => {
    const id = listedRowId(row);
  ...
  if (forceNew) {
    for (const row of exact) pushDelete(row);
    return { keep: null, deleteIds, create: true };
  }

  const keepRow =
    exact.find((row) => listedRowSecret(row) && listedRowId(row)) ||
    exact.find((row) => listedRowSecret(row)) ||
    exact.find((row) => listedRowId(row)) ||
    exact[0] ||
    null;
  const keepId = keepRow ? listedRowId(keepRow) : null;
  for (const row of exact) {
    if (listedRowId(row) && listedRowId(row) !== keepId) pushDelete(row);
  }

  let keep = keepRow ? listedRowSecret(keepRow) : null;
  if (!keep?.key) {
    const named = rows.find((row) => (
      listedRowInGroup(row, groupId)
      && isRelayProbeName(listedRowName(row))
      && listedRowSecret(row)
    ));
    keep = named ? listedRowSecret(named) : null;
  }
  if (!keep?.key) {
    const any = rows.find((row) => listedRowInGroup(row, groupId) && listedRowSecret(row));
    keep = any ? listedRowSecret(any) : null;
  }

  return {
    keep: keep?.key ? keep : null,
    deleteIds,
    create: !keep?.key && exact.length === 0
  };
}
